...
//[slide#title]
icon:tv[] Open Source is a no-brainer (man).
...
//[slide#why]
icon:tv[] Perhaps it goes without saying, but if you want to grow, you need to have an effort that's worth joining.
...
People need a vision they can believe in.
A sense of purpose.
A rally call.
A raison d'être.
...
icon:tv[] Dogfish Head, for example, has a great tagline:
...
//[slide#add-readme]
icon:tv[] Your message needs to be organized, authentic and empathetic.
